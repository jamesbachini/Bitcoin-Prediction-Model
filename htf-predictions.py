...
model = load_model('bitcoin_htf_model.keras')

# Load the scaler used during model training
scaler = MinMaxScaler()
data_sample = pd.read_csv('bitcoin_prices_22a.csv')  # Load sample data to fit the scaler
...
def preprocess_data_for_prediction(data):
    try:
        ...
        seq_length = 12  # Match the sequence length used in model training
        x_input = []
...
async def fetch_preprocess_predict():
    ...
    while True:
        try:
            ...
            if len(df) >= 12:
                recent_data = df.iloc[-12:, 1:]  # Use the last 12 rows for prediction
