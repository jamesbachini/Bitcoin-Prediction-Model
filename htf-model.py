...
from tensorflow.keras.layers import LSTM, Dense, Dropout, GRU, Conv1D, Bidirectional
...
# Generate TA data (uncomment if TA data is needed)
...
# Create a new column for price movement in one minute
data['Price_Movement'] = data[target].shift(-12) - data[target]

# Normalize the features and the new target variable
scaler = MinMaxScaler()
data_scaled = scaler.fit_transform(data[features + ['Price_Movement']].dropna())
...
SEQ_LENGTH = 12  # 12 intervals of 5 seconds = 1 minute
...
# Save the model
model.save('bitcoin_htf_model.keras')
