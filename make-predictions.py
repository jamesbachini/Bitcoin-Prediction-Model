        ...
        try:
            ...
            if len(df) >= 3:
                ...
                if x_input.size > 0:
                    ...
                    logging.info(f"Predicted Price Movement: ${round(predicted_movement, 2)}")
                    logging.info(f"Current Price:    ${round(current_price, 2)}")
                    logging.info(f"Predicted Price:  ${round(predicted_price, 2)}")
                    logging.info(f"Predicted Change: ${round(predicted_movement, 2)}")
                    logging.info(f"Direction:        {'UP' if predicted_movement > 0 else 'DOWN' if predicted_movement < 0 else 'FLAT'}")
